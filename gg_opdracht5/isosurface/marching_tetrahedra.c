...
//	printf("%d -> (%d,%d,%d,%d) = (%d,%d,%d,%d) =>",isovalue,(int)c.value[v0],(int)c.value[v1],(int)c.value[v2],(int)c.value[v3],bitvalue[0],bitvalue[1],bitvalue[2],bitvalue[3]);
	/* Cases of 1 triangle:*/
	// case of 0001 or 1110 ERRORv3
	if(bitvalue[1] == bitvalue[2] && bitvalue[2] == bitvalue[0] && bitvalue[0] != bitvalue[3])
    {//printf("0001|1110\n");
/*    printf("0001: %d <-%d-> %d | %d <-%d-> %d | %d <-%d-> %d\n",
    (int)c.value[v0],isovalue,(int)c.value[v1],
    (int)c.value[v0],isovalue,(int)c.value[v2],
    (int)c.value[v0],isovalue,(int)c.value[v3]
    );*/
