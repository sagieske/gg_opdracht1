...
// add new polylist for textured trees
polys *polylistTreeLeafsTextured = NULL;
...
InitializePolygonlists(void)
{
    ...
    loadPolygonalObject(polylistTreeLeafs, "leaf.obj", texture_names, 0.4, 0.05, 1.7, 0);
...
DrawGLScene(void)
{
    ...
    for (int t = 0; t < 12; t++)
    {
        ...
        int num = rand_float()*5+5;
        for (int i = 0; i <= num; i++)
        {
	       glRotatef(360.0/num,0,1,0);
